...
1. **我以后要用一个 OKX AI 服务，能不能提前订？** 能。通过 **Commit Capacity Quote** 订下一段未来时间。接口已经准备好，欢迎其他 OKX AI ASP 插入。接入是 `POST /execute`，说明在 https://commit.jibai.site/adapter 。
2. **OKX AI 市场上的 ASP 供应商答应了之后到底做得怎么样，能不能留下证据，供下一次复盘？** 能。超时、备用接手、保证金扣罚都留在同一条记录里。这就是稳定交付能力的证据。
